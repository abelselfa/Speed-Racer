...
void Matrix44::rotateAroundAPoint(float angle_in_rad, const Vector3& axis, Vector3 point)
{	
	/*
	Matrix44 T = *this;
	T.traslate(0, 0, 0);*/
	...
	Matrix44 T;
	T.setTranslation(point.x, point.y, point.z);

	Matrix44 R;
	R.rotate(angle_in_rad, axis);

	Matrix44 inverseT;
	inverseT.setTranslation(-point.x, -point.y, -point.z);

	*this = *this * inverseT * R * T;
}
