...
uniform vec3 u_eye;
uniform float u_current_speed;
uniform float u_base_speed;
...
{	
	...
	vec3 ambient = texture2D(u_texture_diffuse, v_uv).xyz * 0.4 + vec3(0, 1, 0.9176) * (((u_current_speed - u_base_speed) / u_base_speed) * 0.3);
	vec3 R = reflect(-L, N);
