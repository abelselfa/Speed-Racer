...
{	
	...
	//vec3 ambient = texture2D(u_texture_diffuse, v_uv).xyz * 0.4 + vec3(1.0, 0.816, 0) * ((u_speed/30) * 0.3);
	vec3 ambient = texture2D(u_texture_diffuse, v_uv).xyz * 0.4 + vec3(0.694, 0.302, 1.0) * ((u_speed/30) * 0.3);
	vec3 R = reflect(-L, N);
